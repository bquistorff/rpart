	  ...
	  double *parm, int *size, int who, double *wt)
{
	...
	countn_te = (int *) ALLOC(maxcat, sizeof(int));
 ...
 */
void
anova(int n, int n_te, double *y[], double *x, double *x_te, int nclass,
     ...
     */
		...
		*/
		...
		while ((j = graycode()) < nclass) {
			tsplit[j] = LEFT;
			left_n += countn[j];
			left_n_te += countn_te[j];
			right_n -= countn[j];
			left_wt += wts[j];
			right_wt -= wts[j];
			left_sum += sums[j];
			right_sum -= sums[j];
			if (left_n >= edge && right_n >= edge && left_n_te>=edge_te && (n_te-left_n_te)>=edge_te) {
				temp = left_sum * left_sum / left_wt +
				right_sum * right_sum / right_wt;
				if (temp > best) {
					best = temp;
